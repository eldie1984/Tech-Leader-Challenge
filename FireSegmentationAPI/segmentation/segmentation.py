...
def segmentacionDeIncendios(fuegos: list[dict], d: float=10, t: float=60) -> tuple[int,list[list[str]]]:
  ...
  for fuego in fuegos[:-1]:
        ...
        for new_fuego in fuegos[number_fire:]:
            ...
            if geodesic_distance(fuego['x'], fuego['y'], new_fuego['x'], new_fuego['y']) <= d and (datetime.strptime(fuego['id'].strip(' ')[:16],'%Y-%m-%dT%H:%M') - datetime.strptime(new_fuego['id'].strip(' ')[:16],'%Y-%m-%dT%H:%M')).total_seconds() <= t:
